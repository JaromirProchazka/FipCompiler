...
#pragma GCC message "bison: do not #define YYSTYPE in C++, use %define api.value.type"
    ...
    struct symbol_type : basic_symbol<by_kind>
    {
...
#endif
      {
        ...
        YY_ASSERT(tok == token::TOK_EOF || (token::TOK_YYerror <= tok && tok <= token::TOK_ARROW) || (token::TOK_COMMA <= tok && tok <= token::TOK_STAR) || tok == token::TOK_EMPH || (token::TOK_DAMP <= tok && tok <= token::TOK_ASGN) || (token::TOK_SEMIC <= tok && tok <= token::TOK_VOID) || (token::TOK_STRUCT <= tok && tok <= token::TOK_SIZEOF) || (token::TOK_IN <= tok && tok <= token::TOK_VERT));
#endif
      }
#if 201103L <= YY_CPLUSPLUS
      ...
      {
#if !defined _MSC_VER || defined __clang__
        YY_ASSERT((token::TOK_IDF <= tok && tok <= token::TOK_TYPEIDF) || tok == token::TOK_STRLIT);
#endif
      }
#if 201103L <= YY_CPLUSPLUS
